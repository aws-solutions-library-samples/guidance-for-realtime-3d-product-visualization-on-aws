...
#
# This file is responsible for emptying an Amazon ECR repo.
set -ex
...
if [ "$supply_cfn_export_key" == "null" ]; then
  export cfn_export_key=$(./build-utils/infra/aws.strip-dashes.sh "${aws_cfn_prefix}-${stage}-${repo_cfn_export_name_suffix}-${deploy_id}")
  echo "lookup cfn export: $cfn_export_key"
else 
  export cfn_export_key="$repo_cfn_export_name_suffix"
